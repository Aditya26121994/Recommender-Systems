...
recommendation_model_path = os.path.join(dirname, 'pickle', 'user_final_rating.pkl')
...
def doRecommendations(username):
    ...
    recommendations = pd.merge(recommendations, df[['id', 'reviews_text']], left_on = 'id', right_on = 'id', how = 'left')

    test_data = tfidf_vectorizer.transform(recommendations['reviews_text'].values.astype('U'))
